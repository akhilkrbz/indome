            ...
            <div class="card">
                <div class="row g-0">
                    <div class="col-md-4 border-end">
                        ...
                        @endphp
                        @if($mainImage)
                            <img id="main-product-image" src="{{ route('product.image', ['product' => $product->id, 'filename' => $mainImage->filename]) }}" class="img-fluid" alt="{{ $product->name }}" style="width: 100%; max-height: 420px; object-fit: cover;">
                        @else
                        ...
                        <div class="row mb-3 row-cols-auto g-2 justify-content-center mt-3">
                            @foreach($images as $image)
                                <div class="col">
                                    <img src="{{ route('product.image', ['product' => $product->id, 'filename' => $image->filename]) }}" width="70" class="border rounded cursor-pointer thumbnail-image" alt="{{ $product->name }}" style="height: 70px; object-fit: cover;" data-full-image="{{ route('product.image', ['product' => $product->id, 'filename' => $image->filename]) }}">
                                </div>
                            @endforeach
                        </div>
                    </div>
